...
description: loom-sdlc plan_review reviewer. Stamped from assets/adversary-plan.prompt.md — edit the template, not this file. Dispatch one task per model via the subagent tool's per-task model override.
...
2. The project's governing docs and locked decisions (AGENTS.md, HANDOVER.md, and any first-principles or northstar docs the plan references).
