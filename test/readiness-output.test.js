...
test("AR8: ready fixture — exact text golden", () => {
	...
	try {
		...
		const expected = [
			...
			"check: config.schema-current pass — config schema is current (schemaVersion 3)",
		...
		const expected = `${JSON.stringify(
			{
				...
				checks: [
					...
					{ id: "config.schema-current", status: "pass", message: "config schema is current (schemaVersion 3)" },
