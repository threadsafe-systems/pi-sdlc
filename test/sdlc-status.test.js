	...
	try {
		for (const format of ["text", "json"]) {
			...
			if (format === "json") {
				...
				assert.match(result.stdout, /check: config\.schema-current pass — config schema is current \(schemaVersion 4\)/);
...
test("CV17: recognised v1 is not-ready with canonical migration remedy and unrelated checks still report", () => {
	...
	try {
		...
		assert.deepEqual(check(report, "config.schema-current"), {
			...
			status: "fail",
			message: "config schema is behind this skill (schemaVersion 1 < 4)",
...
test("CV18: schemaVersion 5 is an error with the canonical newer remedy", () => {
	const dir = gitFixture({ files: { ".pi/sdlc/sdlc.config.json": JSON.stringify({ ...VALID_CONFIG, schemaVersion: 5 }) } });
	try {
		const result = runStatus(["--repo-root", dir, "--format", "json"]);
...
test("CV20: malformed JSON and invalid schemaVersion remain config errors", () => {
	const cases = {
		...
		"junk schemaVersion": { text: JSON.stringify({ ...VALID_CONFIG, schemaVersion: "2" }), message: 'manifest is invalid: schemaVersion must be 4 (got "2")' },
	};
