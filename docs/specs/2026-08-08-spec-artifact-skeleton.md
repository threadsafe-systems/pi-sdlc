...
Status: rev 6 — amended after spec-panel round 5 (round 1: SPEC-R1-01..13; round 2: SPEC-R2-01..09; round 3: SPEC-R3-01..06; round 4: SPEC-R4-01..03; round 5: SPEC-R5-01..03 — all incorporated; see `docs/reviews/spec-review-spec-artifact-skeleton-2026-08-08/consolidated.md`)
...
### AM6 — SAS9 gains the slice's own specification as a permitted change class (PR-panel round-2 adjudication R2-A1, 2026-08-08)

- What: AM5's amended SAS9 still omitted the slice's own specification — `docs/specs/2026-08-08-spec-artifact-skeleton.md` is committed on the branch (self-demonstration: the spec is authored in the skeleton's own form) and revised through its review gates, so the submitted diff necessarily contains it; SAS9's "and nothing else" clause failed the diff as submitted. SAS9 now enumerates the spec document as a tenth permitted class.
- Authority: PR-review round-2 adjudication R2-A1 (sol VERIFIED-GAP on round-1 fix A3).
...
- Preconditions: §4 exists and its current paragraphs (the first paragraph beginning `Produce the Spec doc:`, Premise durability, Dialogue discipline, configuration callout) stay intact and in order.
...
- Signature/shape: one new test file (name chosen at Build), pure offline string assertions over the markdown/test surfaces, asserting exactly:
  ...
  - M2: in `phase-spec.md` §4 — the first paragraph still begins `Produce the Spec doc:`; C2's four canonical rule sentences — rendered as a numbered list, each sentence's line beginning with its literal `1.`/`2.`/`3.`/`4.` marker in order — plus the literal sentence `anything missing is a spec defect`, and the skeleton pointer appear in the paragraph immediately after that first paragraph (adjacency — no paragraph may sit between them) and before the paragraph beginning `**Premise durability.**`; and the paragraphs beginning `**Premise durability.**`, `**Dialogue discipline.**`, and `> **Under your configuration:**` follow, in that order, each still beginning with its anchor sentence — all literal anchor assertions, durable after merge. M2 asserts adjacency, beginnings, and order only; full byte-intactness of §4's existing paragraphs' bodies is a diff-shape claim verified at the PR gate by SAS9 (premise durability: no mechanical assertion against a moving base).
  ...
  - M7: C6's minimality — the IDV19 loop is the only filtered use of `ADVERSARY_PROMPTS` (exempting exactly `"spec"`); the test asserts `ADVERSARY_PROMPTS` as the literal `["plan", "spec", "review"]`; the two sibling loops iterate unfiltered; the `validator-task.prompt.md` assertion remains; and the accompanying comment is present, naming AM1 and AM3 and the re-freeze obligation — all literal content assertions.
...
When–Then: M2 passes — in `phase-spec.md` §4 the first paragraph still begins `Produce the Spec doc:`; all four canonical rule sentences (including the unchanged-context clause in rule 2 and the ratio clause in rule 3), each sentence's line numbered `1.` through `4.` in order, the literal sentence `anything missing is a spec defect`, and the `references/spec-artifact-skeleton.md` pointer appear in the paragraph immediately after the first paragraph; and the paragraphs beginning `**Premise durability.**`, `**Dialogue discipline.**`, and `> **Under your configuration:**` follow in that order. M2 asserts adjacency, beginnings, and order; the existing paragraphs' full intactness (including the first paragraph's) is a diff-shape claim verified at the PR gate by SAS9, not by M2.
Falsify: deleting any rule sentence, either clause, the `anything missing is a spec defect` sentence, or the pointer fails M2; so does dropping or reordering any `1.`–`4.` numbering marker, inserting any paragraph between §4's first paragraph and the rules paragraph, placing the rules paragraph after `**Premise durability.**`, or deleting, reordering, or altering the anchor sentence of any of §4's three existing anchor paragraphs.
...
When–Then: M6 passes — the `FROZEN` array equals C7's pinned list **L3** exactly: 16 entries in L3's exact order, `adversary-spec.prompt.md` absent, every other frozen path present and unreordered.
