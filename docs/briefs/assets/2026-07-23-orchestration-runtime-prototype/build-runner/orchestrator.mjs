...
if (cmd === "init") {
	...
	const out = await new Promise((res, rej) => execFile("node", [join(SDLC_SCRIPTS, "resolve-panel.mjs"), "pr_review", "--author", "anthropic/claude-fable-5", "--track", "irreversible", "--config", SPIKE_CONFIG], (err, stdout, stderr) => (err ? rej(new Error(stderr || err.message)) : res(stdout))));
