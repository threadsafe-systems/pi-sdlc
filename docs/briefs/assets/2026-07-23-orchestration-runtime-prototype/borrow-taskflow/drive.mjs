...
const roster = execFileSync("node", [join(SDLC_SCRIPTS, "resolve-panel.mjs"), "pr_review", "--author", "anthropic/claude-fable-5", "--track", "irreversible", "--config", join(BUILD, "spike-config")], { encoding: "utf8" }).trim().split("\n").filter(Boolean);
...
if (mode === "headless") {
  ...
  const fork = (() => {
    try { return forkRunForResume(result.state, {}); } catch (e) { return { ok: false, errors: [String(e?.message ?? e)] }; }
  })();
