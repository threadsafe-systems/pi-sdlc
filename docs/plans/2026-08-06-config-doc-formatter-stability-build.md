...
T1 (#215) is the only initial frontier task. T2 (#216) has a native `blockedBy` edge on T1 because its committed generated bytes and current-state fingerprint depend on T1's renderer/version. Each task is claimed before work and independently validated before closure.
