---
description: Enter the sdlc Implement phase directly (standalone sdlc:implement)
---
Standalone entrypoint for the **Implement** phase. Thin router; it does not
...
Load `references/phase-implement.md` and run under it; do not duplicate its
mechanics.

## 3. Degradation contract (#38)

Implement's authoritative upstream is **committed tasks/build with named checks**.

- **Upstream absent — in BOTH adoption states:** **always refuse-with-redirect**.
  Tell the user the build breakdown with named checks is missing and to run
  `sdlc:tasks` first, then stop.
- **This entrypoint never fabricates check tables or scenario ids** for absent
  upstream, in any adoption state (the counterfeit-artifact rule).
- **Upstream present:** implement test-first against the committed checks, ending
  each task with the configured `review.tasks` validation (read from current
