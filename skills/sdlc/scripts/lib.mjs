...
// Transitional FS2 compatibility. The merged v2 consumers retire these APIs
// as their callers migrate; keeping them here preserves the pre-migration
// surfaces while T2–T4 land in sequence.
export function readModels(root, explicitPath) {
	const p = explicitPath ?? join(root, ".pi", "sdlc", "sdlc.models.json");
	if (!existsSync(p)) fail(`sdlc: this project requires ${p} to resolve a panel (the skill ships no built-in model roster)`);
	let raw;
	try {
		raw = JSON.parse(readFileSync(p, "utf8"));
	} catch (e) {
...
export function inspectModels(raw) {
	...
	for (const k of Object.keys(raw)) if (!allowed.has(k)) add(k, `unknown key '${k}'`);
	if (raw.author_default !== undefined && (typeof raw.author_default !== "string" || !PM_RE.test(raw.author_default))) add("author_default", "author_default must be provider/model");
	if (raw.rules !== undefined) {
		if (!isPlainObject(raw.rules)) add("rules", "rules must be an object");
		else {
			for (const k of Object.keys(raw.rules)) if (k !== "exclude_author_vendor") add(`rules.${k}`, `unknown rules key '${k}'`);
			if (raw.rules.exclude_author_vendor !== undefined && typeof raw.rules.exclude_author_vendor !== "boolean") add("rules.exclude_author_vendor", "rules.exclude_author_vendor must be boolean");
		}
	}
	...
	for (const phase of PHASES) {
		...
		for (const k of Object.keys(ph)) if (k !== "min_panel" && k !== "prefer") add(`phases.${phase}.${k}`, `unknown key phases.${phase}.${k}`);
